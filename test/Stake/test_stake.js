...
describe("Test stake functionality", async function() {
    ...
    it("Stake action from a single account and check user address stake amount", async() => {
        ...
        let amoutToUnstakeAfterAnotherStakeInSecoundCycle = ethers.utils.formatEther(await user3Reward.getAccWithdrawableStake(user3.address));
        expect(parseFloat(amoutToUnstakeAfterAnotherStakeInSecoundCycle)).to.equal(5000);

        await hre.ethers.provider.send("evm_increaseTime", [60 * 60 * 24])
        await hre.ethers.provider.send("evm_mine")
        let amoutToUnstakeAfterAnotherStakeInThirdCyle = ethers.utils.formatEther(await user3Reward.getAccWithdrawableStake(user3.address));
    ...
    it("Stake action from multiple accounts and check user address stake amount", async() => {
        ...
        let amountStakeInFirstCycle = ethers.utils.formatEther(await user2Reward.getAccWithdrawableStake(user2.address));
        expect(parseInt(amountStakeInFirstCycle)).to.equal(13326);

        await hre.ethers.provider.send("evm_increaseTime", [60 * 60 * 24])
        await hre.ethers.provider.send("evm_mine")

        let amountStakeInSecoundCycleFirstAccount = ethers.utils.formatEther(await user1Reward.getAccWithdrawableStake(user1.address));
        expect(amountStakeInFirtCyclePerAccount).to.equal(amountStakeInSecoundCycleFirstAccount);

        let amountInSecoundCycle = ethers.utils.formatEther(BigNumber.from(await user2Reward.getAccWithdrawableStake(user2.address)).sub(BigNumber.from(await user1Reward.rewardPerCycle(0))));
        expect(amountStakeInFirtCyclePerAccount).to.equal(amountInSecoundCycle);

        let amountStakeInSecoundCycleThirdAccount = ethers.utils.formatEther(await user3Reward.getAccWithdrawableStake(user3.address));
    ...
    it("Multiple stake from multiple accounts ", async() => {
        ...
        let amountStakeInFirstCycleFirstAccount = ethers.utils.formatEther(await user1Reward.getAccWithdrawableStake(user1.address));
        let amountStakeInFirstCycleSeconddAccount = ethers.utils.formatEther(await user2Reward.getAccWithdrawableStake(user2.address));
        let amountStakeInFirstCycleThirdAccount = ethers.utils.formatEther(await user3Reward.getAccWithdrawableStake(user3.address));
        ...
        let totalAmountStakeAccount3 = ethers.utils.formatEther(await user3Reward.getAccWithdrawableStake(user3.address));
        let intermediateValueAccount3 = await user3Reward.getAccWithdrawableStake(user3.address);
        ...
        let totalAmountStakeAccount3Cycle5 = await user3Reward.getAccWithdrawableStake(user3.address);
